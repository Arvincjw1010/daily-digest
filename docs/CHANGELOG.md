...
**问题**：国内AI动态首次推送返回「今日无值得关注的内容」。

**根源**：
- 机器之心 RSS (`jiqizhixin.com/rss`) 返回 0 条（feed 不可用）
- 少数派 (`sspai.com`) 非 AI 内容漏网（「本周看什么」「macOS 存储管理漫谈」「响度战争」「SSTV」「Apple 设计奖」「背包评测」）
- Prompt 过于严格，因国内媒体标题夸张（「叛逃」「吹哨」）而跳过有实质内容的文章

**修复**：
- 移除机器之心 RSS
- exclude_filter 加入 `本周看什么|存储管理|漫谈|响度战争|背包|SSTV|Apple 设计奖`
- Prompt 增加：「国内科技媒体的标题风格如此，正文往往有实质信息。与 AI 无关的内容直接跳过」
- Prompt 放宽内容范围，加入 OpenAI 芯片事件作为示例，增加「AI 商业化案例」「开源项目」「关键人物观点」等类型

**验证结果**：
```
国内AI动态: 4 条（全是 AI 相关）
- OpenAI 芯片负责人跳槽 Anthropic ✅
- 国产开源框架发布 5 分钟 AI 长视频方案 ✅
- AI 培训变现：单节课收入 17 万 ✅
- Hinton 预警：AI 已具备意识 ✅
```
